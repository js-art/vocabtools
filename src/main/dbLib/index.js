...
// let defaultLocation = path.join('local-db');
function setDefaultDataPath() {
  const userDataPath = electron.app.getPath('userData');
  const storagePath = path.join(userDataPath, 'local-db-vocabtools');
  return storagePath;
}
let defaultLocation = setDefaultDataPath();
    ...
    try {
      ...
      const foundRows = rows.filter(
        (row) =>
          ...
          row[field].toString().toLowerCase().includes(keyword.toString().toLowerCase()),
      );
  ...
  if (exists) {
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (Object.entries(where).every(([key, value]) => rows[i][key] === value)) {
